...
public class ProveedorController {
    ...
    @Autowired
    private ProveedorRepositorio service;
    ...
    // Agregar
    @PostMapping
    public ResponseEntity<Proveedor> create(@Valid @RequestBody Proveedor product) {
    	Proveedor created = service.save(product);
        return ResponseEntity.ok(created); // 200 con el producto en JSON
    }

    // Modificar
    @PutMapping("/{id}")
    public ResponseEntity<Proveedor> update(@PathVariable int id,
                                          @Valid @RequestBody Proveedor product) {
